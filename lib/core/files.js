...
class Files {
  ...
  dataPath () {
    return path.join(this.config.dataPath, 'open-node-deployer');
  }
  ...
  absoluteValuesFilePath(name, type) {
    return path.join(this.valuesPath(name), `${type}-node.yaml`);
  }
  relativeValuesFilePath(name, type) {
    // path relative to components to prevent issues with spaces in paths
    return path.join('..', 'deployments', name, 'values', `${type}-node.yaml`);
  }
