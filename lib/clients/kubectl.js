...
class Kubectl {
  ...
  async deleteVolumeClaims(labels=['app=full-node', 'app=rpc-node', 'app=bootstrap-node', 'app=collator-node',
  'app.kubernetes.io/name=grafana', 'app.kubernetes.io/name=prometheus', 'app.kubernetes.io/name=alertmanager']) {
    const promises = [];
    labels.forEach((label) => {
      promises.push(this.cmd(`delete pvc --all-namespaces -l ${label}`));
    });
    return Promise.all(promises);
  }
