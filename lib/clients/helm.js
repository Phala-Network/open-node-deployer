...
class Helm {
  ...
  async delete() {
    ...
    for (const [type, config] of Object.entries(this.config.nodes)) {
      if (config.enabled) {
        promises.push(this._cmd(`uninstall --namespace ${this.namespace} ${this._nodeName(type)}`));
      }
    }
  ...
  async deleteDeps(index) {
    ...
    deps.forEach((dep) => {
      promises.push(this._cmd(`uninstall --namespace ${dep.namespace} ${dep.name}`));
    });
    return Promise.all(promises);
  }
  ...
  async _installPrometheusOperator() {
    return this._installGenericChart('kube-prometheus-stack', 'prometheus-community', MONITORING_NAMESPACE,);
  }
  ...
  async _installGrafana() {
    return this._installGenericChart('grafana', 'grafana', MONITORING_NAMESPACE);
  }

  async _installLoki() {
    return this._installGenericChart('loki-stack', 'grafana', MONITORING_NAMESPACE);
  }
  ...
  async _installOpenNode(type, config) {
    ...
    try {
      ...
      // const cmd = `upgrade --install ${name} -f ${relativeValuesFilePath} --namespace ${this.namespace} ~/Documents/Developer/github.com/Phala-Network/open-node-charts/open-node`;
      console.log(cmd);
      await this._cmd(cmd);
    } catch(e) {
  ...
  async _installOpenNodeBaseServices(index) {
    ...
    // const cmd = `upgrade --install open-node-base-services -f ${relativeValuesFilePath} --namespace ${this.namespace} ~/Documents/Developer/github.com/Phala-Network/open-node-charts/open-node-base-services`;
    console.log(cmd);
    await this._cmd(cmd);
  }
