...
class RemoteCluster {
  ...
  async create() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      const name = nameLib.generate(this.config.name, counter);
      const workers = this.config.remote.clusters[counter].workers;
  ...
  async destroy() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      deletionPromises.push(new Promise(async (resolve) => {
        const domain = this._domain(counter);
        const nodes = {
          full: {
            domain,
            ...this.config.nodes.common,
            ...this.config.nodes.full,
            ...this.config.remote.clusters[counter].nodes.full,
          },
          rpc: {
            domain,
            ...this.config.nodes.common,
            ...this.config.nodes.rpc,
            ...this.config.remote.clusters[counter].nodes.rpc,
          },
          bootstrap: {
            domain,
            ...this.config.nodes.common,
            ...this.config.nodes.bootstrap,
            ...this.config.remote.clusters[counter].nodes.bootstrap,
          },
          collator: {
            domain,
            ...this.config.nodes.common,
            ...this.config.nodes.collator,
            ...this.config.remote.clusters[counter].nodes.collator,
          }
        }
        this.helm[counter].setNodes(nodes);
  ...
  async _saveKubeconfig(counter, name, init=true) {
    ...
    const kubeconfigContent = await this._cmd('output -raw kubeconfig', counter, {verbose: false});
